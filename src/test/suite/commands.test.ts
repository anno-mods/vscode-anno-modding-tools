...
function swapExtension(filePath: string, extension: string) {
  const base = path.basename(filePath);
  return path.join(path.dirname(filePath), base.substring(0, base.lastIndexOf('.')) + extension);
}
...
suite('Extension Test Suite', () => {
  ...
  if (fs.existsSync('../out/test/suite/data')) {
    fs.rmdirSync('../../out/test/suite/data');
  }
  ...
  test('.cf7 commands', async () => {
    ...
    const simpleFc_ = swapExtension(simpleCf7, '_.fc');
    fs.renameSync(swapExtension(simpleCf7, '.fc'), simpleFc_);
    await vscode.commands.executeCommand('anno-modding-tools.convertFcCf7', vscode.Uri.file(simpleFc_));

    const cf7Content = await xml2js.parseStringPromise(fs.readFileSync(swapExtension(simpleCf7, '_.cf7'), 'utf8'));
  ...
  test('.dds commands', async () => {
    ...
    const fakePng_ = swapExtension(fakePng, '_.dds');
    fs.renameSync(swapExtension(fakePng, '.dds'), fakePng_);
    await vscode.commands.executeCommand('anno-modding-tools.ddsToPng', vscode.Uri.file(fakePng_));
    assert(fs.existsSync(swapExtension(fakePng, '_.png')));
  });
});
